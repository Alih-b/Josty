...
Constraint checks against the frozen scenario corpus. Failures emit a failure taxonomy class.

**8/11 constraints passed.**
...
| `all_empty_search` | status | **pass** | `—` | All branches empty without failure yields status=empty (#58). |
...
- `stale_news_day_old_cache` (intended_misleading): stale cached result: age 86400s > 1800s
